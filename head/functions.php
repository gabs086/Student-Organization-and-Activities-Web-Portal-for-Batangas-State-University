			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = $db->query("SELECT * FROM announcement WHERE date = '".$currentDate."'");
						...
						echo '</span>';
						//Hover event popup
						echo '<div id="date_popup_'.$currentDate.'"<a href="javascript:;" onclick="getEvents(\''.$currentDate.'\');"></a>';
						echo '<div class="date_window">';
						echo '<div class="popup_event">('.$eventNum.')</div>';
						echo ($eventNum > 0)?'<br/>':'';
...
function getEvents($date = ''){
	...
	if($result->num_rows > 0){
		...
		while($row = $result->fetch_assoc()){ 
            ...
            $eventListHTML .= '<center><img src="../uploads/'.$row['pic'].'" width="60%"></center><br><hr><br>';
        }
