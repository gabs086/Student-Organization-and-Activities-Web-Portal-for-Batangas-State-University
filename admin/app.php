...
<input type="button" value="List" onclick="window.location='list.php'" class="myButton">
			
		 <h2> Approval & Dispproval for Activities & Events</h2>
...
<table class="tftable">
			...
			<tbody>
			  <?php
	  	include('../connect.php');
		$result = mysql_query("SELECT * FROM studformat WHERE status = 'Approved1' ORDER BY ID DESC");
		while($row = mysql_fetch_array($result))
		{
			...
			echo ' <td style="color:#000"><a href="approve1.php?id='.$row["id"].'&name='.$row["orgname"].'&format='.$row['name'].'" style="color:#000">Approve</a> | <a href="decline4.php?id='.$row["id"].'&name='.$row["orgname"].'&format='.$row['name'].'" style="color:#000" rel="facebox">Considered Revise</a></td>';
			echo '</tr>';
		}
?>
					</tbody>
