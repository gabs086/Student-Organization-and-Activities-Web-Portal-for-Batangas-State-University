...
<select class="form-control" name="campus" required id="myInput" onchange="myFunction1()" style="height:50px">
...
<thead style="color:#FFF">
							...
							<?php
include('../connect.php');
$result = mysql_query("SELECT * FROM assess  WHERE org = '$name' ORDER BY campus DESC");
while($row = mysql_fetch_array($result))
{
			echo '<tr>';
...
<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Filter by Activity.." title="Type in a name">
	<table class="tftable order-table table" id="data-table" border="1">
						<thead style="color:#FFF">
							<th><center>Campus</th>
							<th><center>Name of Student</th>
								<th><center>SR-Code </th>
							<th><center>Year</th>
							<th><center>Section</th>
							<th><center>Department</th>	
							<th><center>Contact Number</th>
							<th><center>Activity to Join</th>
		...
		include('../connect.php');
		$result = mysql_query("SELECT * FROM recordassess WHERE campus = '$campus' ORDER BY campus ASC");
		while($row = mysql_fetch_array($result))
		{
			echo '<tr>';
		
			echo '<td valign="top"><font color="#000">'.$row['campus'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['namee'].'</td>';

			echo '<td valign="top"><font color="#000">'.$row['sr'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['year'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['section'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['department'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['connum'].'</td>';
			echo '<td valign="top"><font color="#000">'.$row['activity'].'</td>';
			echo '</tr>';
		}
